...
public class NotificationService {
     ...
     * 
     * @param senderId        ID người like
     * @param receiverId      ID chủ bài viết
     * @param senderName      Tên người like
     * @param senderAvatarUrl Avatar URL người like
     * @param postId          ID bài viết
     * @return Notification đã được lưu
     */
    @Transactional
    public Notification createLikeNotification(
            String senderId,
            String receiverId,
            String senderName,
            String senderAvatarUrl,
        ...
        Notification saved = notificationRepository.save(notification);
        log.info("Created like notification: {} -> {} for post {}", senderId, receiverId, postId);
        return saved;
    }

    /**
     * Tạo notification khi có người bình luận bài viết.
     * 
     * @param senderId        ID người bình luận
     * @param receiverId      ID chủ bài viết
     * @param senderName      Tên người bình luận
     * @param senderAvatarUrl Avatar URL người bình luận
     * @param postId          ID bài viết
     * @param commentPreview  Preview nội dung bình luận
     * @return Notification đã được lưu
     */
    @Transactional
    public Notification createCommentNotification(
        ...
        // Truncate comment preview nếu quá dài
        String preview = commentPreview;
        ...
        Notification notification = Notification.builder()
                ...
                .type(Type.COMMENT)
                .postId(UUID.fromString(postId))
                .message(preview != null ? "\"" + preview + "\"" : "đã bình luận về bài viết của bạn")
                .isRead(false)
                .createdAt(Instant.now())
                .build();

        Notification saved = notificationRepository.save(notification);
        log.info("Created comment notification: {} -> {} for post {}", senderId, receiverId, postId);
        return saved;
    }
